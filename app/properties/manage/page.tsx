...
import {
  ...
  MapPin, Loader2, CheckCircle, AlertCircle, ArrowLeft, X
} from "lucide-react"
...
const CAT_COLORS: Record<string, string> = {
  "Stand": "bg-sky-100 text-sky-800",
  "Airbnb": "bg-rose-100 text-rose-800",
  "Land Development": "bg-amber-100 text-amber-800",
...
function sortByDate(list: Property[]): Property[] {
...
export default function PropertyManagerPage() {
  ...
  const loadProperties = useCallback(async () => {
    ...
    try {
      ...
      setProperties(sortByDate(Array.isArray(data) ? data : []))
    } catch {
      setToast({ msg: 'Failed to load properties', type: 'error' })
      setTimeout(() => setToast(null), 3500)
    } finally {
      setLoading(false)
    }
  ...
  function selectProperty(property: Property) {
    ...
    setForm({
      ...
      date: formatDateForInput(property.date),
  ...
  async function handleSave(andPublish = false, silent = false) {
    ...
    if (!form.category) { showToast('Please select a property type', 'error'); return }
    ...
    const payload = {
      ...
      image: form.image.trim() || '/images/rearesidential.webp',
      date: form.date,
    ...
    try {
      const res = await fetch(isNew ? '/api/properties' : `/api/properties/${selectedId}`, {
      ...
      if (isNew) {
        setProperties(prev => sortByDate([saved, ...prev]))
      } else {
        setProperties(prev => sortByDate(prev.map(p => p.id === saved.id ? saved : p)))
      }
  ...
  async function handleDelete() {
    ...
    try {
      await fetch(`/api/properties/${idToRemove}`, { method: 'DELETE' })
      setShowDeleteModal(false)
      setSelectedId(null)
      setIsNew(false)
      setProperties(prev => prev.filter(p => p.id !== idToRemove))
      showToast('Listing unpublished', 'success')
    } catch {
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-background font-sans">
      ...
      <header className="flex h-auto min-h-14 shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border bg-primary px-3 py-2 sm:px-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/properties" className="flex items-center gap-1.5 text-primary-foreground/60 hover:text-primary-foreground text-sm transition-colors">
            <ArrowLeft className="h-4 w-4" />
            <span className="hidden sm:inline">Back to site</span>
          </Link>
          <span className="hidden sm:inline text-primary-foreground/30">|</span>
          <span className="font-serif text-sm font-bold text-accent sm:text-base">🏠 Property Manager</span>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
              ...
              visibleProperties.map(property => (
                <button
                  ...
                  className={`w-full border-b border-border px-4 py-3 text-left transition-colors hover:bg-muted/50 ${
                    selectedId === property.id ? 'border-l-2 border-l-accent bg-accent/5' : 'border-l-2 border-l-transparent'
                  }`}
                >
                  <div className="mb-1.5 flex items-center gap-1.5">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${CAT_COLORS[property.category] || 'bg-gray-100 text-gray-700'}`}>
                      {property.category}
                    </span>
                    {property.featured && <Star className="h-3 w-3 fill-amber-400 text-amber-400" />}
                    {!property.published && <span className="rounded bg-red-100 px-1.5 text-[10px] font-bold text-red-600">DRAFT</span>}
                  </div>
                  <p className="mb-1 line-clamp-2 text-sm font-semibold leading-snug text-foreground">{property.title}</p>
        ...
        <main className={`${mobileView === 'editor' ? 'flex' : 'hidden'} md:flex flex-1 flex-col overflow-hidden`}>
          {!hasSelection ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-4 p-6 text-center">
              ...
              <h2 className="font-serif text-2xl font-bold text-foreground">Select or create a listing</h2>
              <p className="max-w-xs text-sm text-muted-foreground">
                Choose an existing property from the panel, or add a new stand, Airbnb, development, or other listing.
              </p>
                  ...
                  <div className="mx-auto max-w-3xl space-y-5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. Luxury stand in Borrowdale"
                    ...
                    <div>
                      <label className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-muted-foreground">
                        ...
                        <span className="ml-2 font-normal normal-case text-muted-foreground/60">shown on listing cards</span>
                      </label>
                      <textarea
                        value={form.excerpt}
                    ...
                    <div>
                      ...
                      <RichTextEditor
                        ...
                        placeholder="Describe the property, location, features…"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-3">
                        ...
                        <input
                          ...
                          placeholder="/images/property.jpg  or  https://…"
                    ...
                    <div className="flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card p-4 sm:gap-8">
                      <label className="flex cursor-pointer items-center gap-3">
                        ...
                        <span className="flex items-center gap-1.5 text-sm font-medium">
                          <Star className="h-4 w-4 text-amber-400" /> Featured listing
                        </span>
                      </label>
              ...
              {mode === 'preview' && (
                <div className="flex-1 overflow-y-auto">
                  <div className="relative flex min-h-[280px] items-end bg-primary">
                    ...
                    <div className="relative z-10 max-w-3xl px-4 pb-8 pt-16 sm:px-8 sm:pb-10 sm:pt-20">
                      ...
                      <div className="flex items-center gap-4 text-sm text-primary-foreground/70">
                        <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> Listed {form.date ? formatDateForDisplay(form.date) : '—'}</span>
                  ...
                  <div className="mx-auto max-w-3xl px-4 py-8 sm:px-8 sm:py-10">
                    ...
                    <div
                      dangerouslySetInnerHTML={{ __html: form.content || '<p>Add your property description…</p>' }}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-2xl bg-background p-8 shadow-2xl">
            ...
            <h3 className="mb-2 text-center font-serif text-xl font-bold">Unpublish this listing?</h3>
            <p className="mb-6 text-center text-sm text-muted-foreground">
              The listing will be hidden from the site. You can republish from the database or edit it here later.
            </p>
            <div className="flex gap-3">
              <button
